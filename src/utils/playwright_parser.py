...
>>>>>>> 0356ec0 (feat: Добавить enterprise-grade retry логику для SMS API и OTP)
                ...
                if field_type == 'otp' and var_name.startswith('otp'):
                    ...
                    code_lines.append('')
                    code_lines.append(f'# ========== УМНЫЙ ВВОД OTP (3 попытки + fallback селекторы) ==========')
                    code_lines.append(f'otp_entered = False')
                    code_lines.append(f'for otp_attempt in range(1, 4):  # 3 попытки')
                    code_lines.append(f'    try:')
                    code_lines.append(f'        print(f"[OTP] Попытка {{otp_attempt}}/3 ввести OTP...")')
                    code_lines.append(f'        # Увеличенный таймаут для OTP поля (30 сек)')
                    code_lines.append(f'        await page.{selector_code}.wait_for(state="visible", timeout=30000)')
                    code_lines.append(f'        await page.{selector_code}.scroll_into_view_if_needed()')
                    ...
                    code_lines.append(f'        print(f"[OTP] [OK] OTP введен: {{data_row[\'{var_name}\']}}")')
                    code_lines.append(f'        otp_entered = True')
                    code_lines.append(f'        break  # Успех - выходим из цикла')
                    code_lines.append(f'    except Exception as e:')
                    code_lines.append(f'        print(f"[OTP] Попытка {{otp_attempt}} не удалась: {{e}}")')
                    code_lines.append(f'        if otp_attempt < 3:')
                    code_lines.append(f'            await page.wait_for_timeout(3000)  # Пауза перед следующей попыткой')
                    code_lines.append(f'if not otp_entered:')
                    code_lines.append(f'    print("[OTP CRITICAL] Не удалось ввести OTP после всех попыток!")')
                    code_lines.append('')
