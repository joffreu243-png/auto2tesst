    ...
    def _generate_converted_code(self, actions: List[Dict], url: str) -> str:
        ...
        for action in actions:
            if action['type'] == 'goto':
                ...
                code_lines.append(f'try:')
                code_lines.append(f'    # Ждем появления поля (до 20 сек)')
                code_lines.append(f'    await page.{selector_code}.wait_for(state="visible", timeout=20000)')
                code_lines.append(f'    # Скролл к полю перед заполнением')
                code_lines.append(f'    await page.{selector_code}.scroll_into_view_if_needed()')
                code_lines.append(f'    await page.wait_for_timeout(500)  # Пауза после скролла')
                code_lines.append(f'    # Очистить поле перед вводом')
                code_lines.append(f'    await page.{selector_code}.clear()')
                code_lines.append(f'    # Имитация человеческого ввода с случайной задержкой между символами')
                code_lines.append(f'    delay = random.randint(50, 150)  # Задержка 50-150 мс между символами')
